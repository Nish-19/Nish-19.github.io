---
layout: page
title: Experience
permalink: /experience/
---

## Research Intern / Indian Institute of Science Bangalore
*May 2020 - Sept 2020*        
**Guide** : Prof. R. Venkatesh Babu

Worked in the Video Analytics Laboratory (VAL), department of Computational Data Sciences (CDS - IISc). Experimented with the state-of-the-art semi-supervised models for Adversarial Robustness. Came up with my own way of generating unlabeled data using DC-GANs and DE-GANs for improving adversarial robustness.

## Undergraduate Researcher / AI - ML - NLP lab IIT Patna
*July 2019 - Present*       
**Guides** : Dr. Asif Ekbal and Prof. Phuspak Bhattacharya

I am working on misinformation detection. Worked previously on document level novelty detection and building a chatbot for the academic portal of IIT - Patna. I Have submitted papers to top conferences and journals.

## Data Science Intern / TildeHat Bangalore
*Dec 2019*       
**Advisor** : Dr. Om Deshmukh

Worked on unsupervised data retrieval. I designed a complete end-to-end module for useful information extraction from
resumes. I implemented unsupervised algorithms and hierarchical classifier approaches for data-retrieval.

## Undergraduate Researcher / Data Analytics and Networks lab IIT Patna
*January 2020 - May-2020*        
**Guide** : Dr. Joydeep Chandra

Worked on Reddit Drug Abuse Detection. Came up with a novel multi-head attention model to classify comments collected from reddit as motivational or not. Classification done on sub-categories like altruism, bad advice, hope.

## Position of Responsibilities
* **Coordinator Njack Machine Learning** - I promote Machine Learning among the students of IIT-P by guiding them towards research, industry and the competitive aspects of ML.
* **Head Industrial Relations - E-Cell IIT-Patna** - I manage a team of volunteers and sub-coordinators. Play a pivotal role in establishing contacts with eminent personalities from the industry.
